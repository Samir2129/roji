...
/* ==========================
   TYPEWRITER
   Starts ONLY after surprise unlock.
   ========================== */

let typewriterStarted = false;

function startTypewriter() {
    if (typewriterStarted || !messageEl) return;

    typewriterStarted = true;

    const originalText = messageEl.dataset.message || messageEl.textContent.trim();
    ...
    function typeWriter() {
        if (index < originalText.length) {
            messageEl.textContent += originalText.charAt(index);
            index++;
...
if (openGift && giftSection) {
    openGift.addEventListener("click", () => {
        if (openGift.disabled || Date.now() < BIRTHDAY_TIME) return;
...
function updateConfetti() {
    ...
    for (let i = confetti.length - 1; i >= 0; i--) {
        ...
        if (confetti[i].y > window.innerHeight + 30) {
            confetti.splice(i, 1);
        }
    }
}
...
function startBirthdayCelebration() {
    ...
    if (titleEl) {
        titleEl.textContent = "🎉 Happy Birthday Rojina 🎂";
    }

    if (subtitleEl) {
        subtitleEl.textContent = "Wishing You Endless Happiness 💖";
    }

    /*
       Browsers may block autoplay.
       We try automatically at midnight.
       If blocked, the Play Music button remains available.
    */
    playMusic();
...
/* ==========================
   DESKTOP SPARKLES
   Throttled to avoid excessive DOM creation.
   ========================== */
...
/*
   IMPORTANT:
   The celebration automatically starts at:
   29 August 2026, 12:00 AM
*/
